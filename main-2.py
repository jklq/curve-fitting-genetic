...
def plot_model_vs_actual(X, actual_answers, prediction_model):

    # Create a new figure before plotting
    plt.figure(figsize=(12, 6))

    # Calculate predictions using the model
    model_predictions = [calculateNode(prediction_model, x) for x in X]

    # Actual values
    plt.plot(X, actual_answers, label='Actual', color='blue')

    # Model predictions
    plt.plot(X, model_predictions, label='Model Prediction',
...
num_generations = 10  # Specify the number of generations here
...
for gen in range(num_generations):
    ...
    plt.close()
    plot_model_vs_actual(X, answers, top_model)
